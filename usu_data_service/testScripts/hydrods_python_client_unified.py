...
class HydroDS(object):
    ...
    def subset_netcdf_by_coordinates(self, input_netcdf, output_netcdf, leftX, topY, rightX, bottomY,
                                     in_Xcoord, in_Ycoord, save_as=None):
        """
        Subset netcdf data based on coordinates
        ...
        Example usage:
            hds = HydroDS(username=your_username, password=your_password)
            response_data = hds.subset_netcdf_by_coordinates(input_netcdf=provide_input_netcdf_static_file_name_or_url_file_path_here,
                                              ...
                                              output_netcdf='subset_netcdf_to_spawn.nc')
        ...
        url = self._get_dataservice_specific_url('subsetnetcdfbycoordinates')
        payload = {'input_netcdf': input_netcdf, 'output_netcdf': output_netcdf, 'leftX': leftX, 'topY': topY,
                   'rightX': rightX, 'bottomY': bottomY, 'in_Xcoord': in_Xcoord, 'in_Ycoord': in_Ycoord
                  }
    ...
    def concatenate_multiple_netcdf(self, output_netcdf, inout_timeName = 'time', input_netcdf_list_json=None, save_as=None):
        """
        :param input_netcdf_list_json: either a file name for a supported netcdf data resource (static data file) on HydroDS api server
                             or url file path for a netcd file that the user owns on the HydroDS server
        :type input_netcdf_list_json: json
        :param inout_timeName:
        :type inout_timeName: string
        :param output_netcdf: name for the output netcdf file (if there is file already with the same name it will be
                              overwritten)
        :param save_as: (optional) file name and file path to save the generated netcdf file locally
        :type save_as: string
        ...
        Example usage:
            hds = HydroDS(username=your_username, password=your_password)
            response_data = hds.concatenate_multiple_netCDF(input_netcdf=provide_input_netcdf_static_file_name_or_url_file_path_here,
                                              ...
                                              output_netcdf='subset_netcdf_to_spawn.nc')
        ...
        url = self._get_dataservice_specific_url('concatenatemultiplenetcdf')
        payload = {'output_netcdf': output_netcdf, 'inout_timeName': inout_timeName,
                   'input_netcdf_list_json':input_netcdf_list_json
                  }
    ...
    def subset_netcdf_by_datetime(self, input_netcdf, output_netcdf, startDateTime, endDateTime,
                                  dT, inout_timeName, save_as=None):
        """
        Subset netcdf data based on datetime (formated datetime)
        ...
        try:
            start_date_value = datetime.datetime.strptime(startDateTime, DATE_FORMAT)
        except ValueError:
            raise HydroDSArgumentException("start_date must be a string in the format of 'yyyy/mm/dd/ H'")

        try:
            end_date_value = datetime.datetime.strptime(endDateTime, DATE_FORMAT)
        except ValueError:
            raise HydroDSArgumentException("end_date must be a string in the format of 'yyyy/mm/dd H'")
        ...
        url = self._get_dataservice_specific_url('subsetnetcdfbydatetime')
        payload = {'input_netcdf': input_netcdf, 'output_netcdf': output_netcdf,
                   'startDateTime': startDateTime, 'endDateTime': endDateTime, 'dT': dT, 'inout_timeName': inout_timeName
                  }
    ...
    def subset_project_timespaceresample_netcdf_to_referencenetcdf(self, input_netcdf, reference_netcdf, output_netcdf,
        inout_varName, ref_varName, in_epsgCode=None, tSampling_interval=3, start_Time = 0.0, dTin = 1.0, inout_timeName = 'time',
        time_unitString ='hours since 2008-10-01 00:00:00 UTC', in_Xcoord = 'lon_110', in_Ycoord='lat_110', save_as=None):
        """
        :param input_netcdf: either a file name for a supported netcdf data resource (static data file) on HydroDS api server
                             or url file path for a netcd file that the user owns on the HydroDS server
        :type input_netcdf: string
        ...
        Example usage:
            hds = HydroDS(username=your_username, password=your_password)
            response_data = hds.subset_project_timespaceresample_netcdf_to_referencenetcdf
                                 (input_netcdf=provide_input_netcdf_static_file_name_or_url_file_path_here,
                                              ...
                                              output_netcdf='subset_netcdf_to_spawn.nc')
        ...
        url = self._get_dataservice_specific_url('subsetprojecttimespaceresamplenetcdftoreferencenetcdf')
        payload = {'input_netcdf': input_netcdf, 'reference_netcdf': reference_netcdf, 'output_netcdf': output_netcdf,
                   'inout_varName': inout_varName, 'ref_varName': ref_varName, 'in_epsgCode': in_epsgCode,
                   'tSampling_interval': tSampling_interval, 'start_Time': start_Time, 'dTin': dTin,
                   'inout_timeName': inout_timeName, 'time_unitString': time_unitString, 'in_Xcoord': in_Xcoord, 'in_Ycoord': in_Ycoord
                  }
    ...
    def compute_average_of_two_netcdf_vars(self, input_netcdf1, input_netcdf2, output_netcdf, varName1, varName2,  varNameO,
                                       varOut_unit = 'm/s', varOut_longName='Average wind speed at height 10 m', save_as=None):
        """
        :param input_netcdf: either a file name for a supported netcdf data resource (static data file) on HydroDS api server
                             or url file path for a netcd file that the user owns on the HydroDS server
        :type input_netcdf: string
        ...
        url = self._get_dataservice_specific_url('computeaverageoftwonetcdfvars')
        payload = {'input_netcdf1': input_netcdf1, 'input_netcdf2': input_netcdf2, 'output_netcdf': output_netcdf,
                   'varName1': varName1, 'varName2': varName2, 'varNameO': varNameO,
                   'varOut_unit': varOut_unit, 'varOut_longName': varOut_longName
                  }
    ...
    def subset_nldas_forcing(self, output_netcdf, leftX, topY, rightX, bottomY,
                      startDateTime, endDateTime, dT, in_Xcoord, in_Ycoord, inout_timeName, save_as=None):
        """
        Subset netcdf data based on a reference raster
        ...
        url = self._get_dataservice_specific_url('subsetnldasforcing')
        payload = {'output_netcdf': output_netcdf, 'leftX': leftX, 'topY': topY, 'rightX': rightX, 'bottomY': bottomY,
                   'startDateTime': startDateTime, 'endDateTime': endDateTime, 'dT': dT, 'in_Xcoord': in_Xcoord,
                   'in_Ycoord': in_Ycoord, 'inout_timeName': inout_timeName
                   }
    ...
    def adjust_for_elevation_Forcing(self, input_netcdf, output_netcdf, varName, input_raster, target_raster,
                                     time_var_name='time', baseDateTime='1980/01/01 0', timeUnits='days', varCode= 'temp', save_as=None):
        """
        ajusts temerature values with lapse rate based on elevaton
        :param input_netcdf: input
        :param output_netcdf: output
        ...
        url = self._get_dataservice_specific_url('adjustforelevationforcing')
        payload = {'output_netcdf': output_netcdf, 'input_netcdf': input_netcdf, 'varName': varName,
                   'input_raster': input_raster, 'target_raster': target_raster, 'time_var_name': time_var_name,
                   'baseDateTime': baseDateTime, 'timeUnits': timeUnits, 'varCode': varCode
                  }
    ...
    def concatenate_netcdf(self, input_netcdf1_url_path, input_netcdf2_url_path, output_netcdf, inout_timeName='time', save_as=None):
        """
        ...
        :param output_netcdf: name of the output (concatenated) netcdf file (if there is file already with the same name
                              it will be overwritten)
        :type output_netcdf: string
        :param inout_timeName: name of the output (concatenated) netcdf file time variable)
        :type inout_timeName: string
        ...
        payload = {"input_netcdf1": input_netcdf1_url_path, "input_netcdf2": input_netcdf2_url_path,
                   'output_netcdf': output_netcdf, 'inout_timeName':inout_timeName}
...
######################Start of TOPNET function ####################
    ...
    def download_streamflow(self, usgs_gage, start_year, end_year, output_streamflow, save_as=None):
        ...
        if not self._is_file_name_valid(output_streamflow, ext='.dat'):
            raise HydroDSArgumentException("{file_name} is not a valid file name".format(file_name=output_streamflow))
    ...
    def delineate_watershed_peuker_douglas(self, input_raster_url_path, threshold, peuker_min_threshold, peuker_max_threshold, peuker_number_threshold,
                                output_watershed_raster,output_outlet_shapefile,output_treefile,output_coordfile,output_streamnetfile,output_slopearea_raster,output_distance_raster,
...
# 5.1.19 This doesn't have corresponding code in view so commented out (for now?)
